...
/***/ function(module, exports, __webpack_require__) {
	...
	$(function () {
	    ...
	    var survey = __webpack_require__(24);
...
/***/ function(module, exports, __webpack_require__) {
	...
	$.fn.renderCodeText = function () {
	    ...
	    if ($input.length) {
	        $input.renderText();
	        $input.after('<button type="button" class="yiqi-code-button">发送验证码</button>')
...
/***/ function(module, exports, __webpack_require__) {
	...
	$.fn.quicktouchclick = function (handle) {
	    var hold,
	        lock = false,
	        self = this;
	    this.on('touchstart', function () {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var errorAction;
	var server =  true ? 'http://test.yi-gather.com:1717/' : window.location.origin + '/';
	var defaultData = {
	    type: 'web'
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	//cookie.set('userid', '1446189615880d952ede961ae452e9151');
	//cookie.remove('userid');
	var device = __webpack_require__(19);
	var util = __webpack_require__(3);
	var server = __webpack_require__(11);
	var user = __webpack_require__(20);
	var pay = __webpack_require__(21);
	var args = util.getArgs();
	var handleForm = __webpack_require__(23);
	//alert(user.userid);
	...
	var searchTickets = {
	    single: function () {
	        server.post('v20/activity/searchrestticket', {
	            id: activityId
	        }).done(function (data) {
	            $button.html('提交：余票' + _.values(data)[0].rest);
	        });
	    },
	    set: function () {
	        server.post('v20/activity/searchrestticket', {
	            ...
	            _.each(data, function (obj, id) {
	                server.post('v20/activity/getactivity', {
	                    id: id
	                }).done(function (data) {
	                    activities.push(data);
	                    $buys.filter('[value="' + obj.title + '"]').val(id)
	                        .nextAll('.yiqi-input-label').html(obj.title + '：余票' + obj.rest);
	                })
	            })
	        });
	    }
	...
	var signActivity = {
	    single: function () {
	        var chain = activities.cost === 0 ? orderActivity.free(activities) : orderActivity.cost(activities);
	        return chain.then(function () {
	            if (orders.length) {
	                payAction.handle(orders);
	            }
	        });
	    },
	    set: function () {
	        var defer = $.Deferred();
	        var buyActivity = _.filter(activities, function (obj) {
	            return _.contains(buys, obj.id)
	        });
	        async.each(buyActivity, function (item, cb) {
	            var chain = item.cost === 0 ? orderActivity.free(item) : orderActivity.cost(item)
	            chain.always(function () {
	                cb && cb(null);
	            });
	        }, function () {
	            if (orders.length) {
	                payAction.handle(orders).then(function () {
	                    defer.resolve();
	                });
	            }
	        });
	...
	var orderActivity = {
	    free: function (entity) {
	        return server.post('v20/activity/free_sign_activity', {
	            activityid: entity.id,
	            questions: userText,
	            userid: user.userid
	        }).then(function (data) {
	            alert(entity.title + '报名成功!');
	        }).fail(function (err) {
	            alert(entity.title + '报名失败,' + err);
	        });
	    },
	    cost: function (entity) {
	        var userid = user.userid;
	        return server.post('v20/order/share_create_order', {
	            userid: userid,
	...
	var $body = $('body');
	var payAction = {
	    handle: function (orders) {
	        return device.weixin ? this.weixin(orders) : this.alipay(orders[0]);
	    },
	    weixin: function (orders) {
	        return pay.weixinPayProxy(orders);
	    },
	    alipay: function (order) {
	        return pay.alipay(order.id, user.userid)
	            .then(function (data) {
	                //data = data.replace(/<form/, '<form target="_blank" ');
	                $body.append(data);
	            });
	    }
	}
	...
	function init(id) {
	    activityId = id;
	    return server.post('v20/activity/getactivity', {
	        id: id
	    }).done(function (data) {
	        activities = data;
	        activityType = data.activityType;
	        searchTickets[activityType]();
	    })
	}
	...
	function handleBuys() {
	    var buy = [];
	    if ($buys.length) {
	        $buys.filter(':checked').each(function () {
	            buy.push(this.value);
	        });
	    } else {
	        buy.push(activityId);
	    }
	    buy = buy.length ? buy : false;
	    return buy;
	}

	var deviceAciton = function () {
	    var data = JSON.stringify({
	        questions: userText,
	        buy: buys[0] || buys.join(',')
	    });
	    var action = {
	...
	$button.on('touchstart', function (event) {
	    ...
	    if (device.yiqi) {
	        ...
	        submitLock = true;
	        signActivity[activityType]()
	            .always(function () {
	                submitLock = false;
	                orders = [];
	            });
	    } else {
	        alert('请填写你的基本信息或选择购买的内容');
	    }
	});


	module.exports = function () {
	    $form.find('[name="name"], [name="phone"], [name="email"]').parents('.text-area').hide();
	    user.ready(function () {
	        if (user.validUser()) {
...
/***/ function(module, exports) {
	...
	if (device.android && device.version<5) {
	...
	device.androidCall = function (method, params) {
	    ...
	    if (yiqi && yiqi[method]) {
	        if (params) {
	            params instanceof Array ? yiqi[method].apply(this, params) : yiqi[method](params);
	        } else {
	            yiqi[method]();
	        }
	    }
...
/***/ function(module, exports, __webpack_require__) {
	...
	if (id) {
	 ...
	 true ? console.log('now userid is ' + id) : null;
	module.exports = {
	    userid: id,
	    getEntity: function () {
	        return user;
...
/***/ function(module, exports, __webpack_require__) {
	...
	if (device.weixin && !device.yiqi) {
	    server.post('v20/user/get_config_data', {
	        url:  true ? 'www.yi-gather.com/dispatch/test' + window.location.pathname : window.location.href
	    }).done(function (data) {
	        wx.config({
	            debug: (true),
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = function () {
	    ...
	    $('.submit-button').click(function (event) {
	        ...
	        server.post('v20/answer/createanswer', {
	            data: data
	        })
	            .done(function (data) {
	                alert('问卷提交成功');
