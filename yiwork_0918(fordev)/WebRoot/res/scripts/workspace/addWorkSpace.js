     ...
     { 
    	url:root+"v20/upload/uploadimg",//用于文件上传的服务器端请求的Action地址 
     	...
     	success:function(json)  { 
    	 	 	   ...
    	 	 	   if(json.cord==0){
    	 	 		   $("#spacelogo").val(root+"v20/download/img?type=web&path="+json.data);
...
function sendFile1()
{
     ...
     { 
    	url:root+"v20/upload/uploadimg",//用于文件上传的服务器端请求的Action地址 
     	type:"post",//请求方法 
     	secureuri:false,//一般设置为false 
     	fileElementId:'coverImg1',//文件id属性  &amp;lt;input type="file" id="upload" name="upload" /&amp;gt; 
     	dataType:"JSON",//返回值类型 一般设置为json,一定要大写,否则可能会出现一些bug 
     	success:function(json)  { 
    	 	 	   json=JSON.parse(json)  //手动转json
    	 	 	   	//successNum=successNum+1;
    	 	 	   if(json.cord==0){
    	 	 		   $("#bannerimg").val(root+"v20/download/img?type=web&path="+json.data);
    	 	 		   $("#coversrc1").attr("src",root+"v20/download/img?type=web&path="+json.data);
    	 	 	   }
		 } 
     }); 
}
