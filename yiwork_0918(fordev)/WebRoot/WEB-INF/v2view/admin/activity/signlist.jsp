...
<div class="row row-flow">
            ...
            <div class="col-xs-12">
                ...
                <form id="mainForm" name="mainForm" action="<%=basePath %>v20/admin/activity/activitysignlist" method="post">
								...
								<c:if test="${page.totalPage gt 0 }">
									<c:forEach items="${page.result }" var="userinfo" varStatus="vs">
										<tr>
											...
											<td>${userinfo.unum}</td>
											<%-- <td><fmt:formatDate value="${userinfo.getCompanyurl }" pattern="yyyy-MM-dd HH:mm:ss"/></td> --%>
											<td>${userinfo.companyurl}</td>
										</tr>
									</c:forEach>
						</c:if>
                    </tbody>
